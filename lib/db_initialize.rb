...
searches = db.execute('CREATE TABLE IF NOT EXISTS searches (id integer primary key autoincrement, search_url varchar(255), category varchar(64), region varchar(64), min_price integer, max_price integer, keyword varchar(64), created_at default current_timestamp, user_id integer, foreign key(user_id) references users(id));')
